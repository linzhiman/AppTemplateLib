...
{
    //iOS7 deque.clear()内存还给了allocator（内存池，deque的内存复用），但allocator没有被释放？
//    fprintf(stderr, "【C++ print】: m_sentenceList before %zu \n", m_sentenceList.size());
    m_sentenceList.clear();
//    _SENTENCE_LIST::allocator_type allocator = m_sentenceList.get_allocator();
//    fprintf(stderr, "【C++ print】: m_sentenceList after %zu \n", m_sentenceList.size());
	return;
}
...
{
	pSentenceItem->ccSentence.assign(sentence);
	...
	if ((sentenceFlags & (SENTENCE_SUPPRESS_ABC_FLAG|SENTENCE_SUPPRESS_T9_FLAG)) != (SENTENCE_SUPPRESS_ABC_FLAG|SENTENCE_SUPPRESS_T9_FLAG))
	{
		...
		{
			std::w16string word;
			_enumWordType wordType(_wordtype_dummyOther);
...
void CT9SearchEngine::_adjustSentence(std::w16string* pSentence)
{
...
{
#ifdef _DEBUG
	pWordItem->ccWord.assign(word, wordLen);
...
size_t CT9SearchEngine::_takeFirstWord(LPCWSTR sentence, unsigned sentenceFlags, size_t sentencePos, std::w16string* pWord, _enumWordType* pWordType)
{
	//skip space chars
...
void CT9SearchEngine::_adjustKeyAndSearchingFlags(std::w16string* pKey, unsigned* pSearchingFlags)
{
