...
//        ~_WORD_ITEM() {
//            while(!abcWordList.empty()) {
//                std::string string = abcWordList.back();
//                std::string().swap(string);
//                abcWordList.pop_back();
//            }
//            abcWordList.clear();
//        }
...
private:
	...
	static void _adjustSentence(std::w16string* pSentence);

	static void _makeWordItem(LPCWSTR word, size_t wordLen, _enumWordType wordType, _WORD_ITEM* pWordItem);

	static size_t _takeFirstWord(LPCWSTR sentence, unsigned sentenceFlags, size_t sentencePos, std::w16string* pWord, _enumWordType* pWordType);
	...
	*/
	...
	static void _adjustKeyAndSearchingFlags(std::w16string* pKey, unsigned* pSearchingFlags);
